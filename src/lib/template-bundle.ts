...
export type TemplateStatus = 'draft' | 'previewed' | 'saved';
...
export const getBundleTemplate = (files: TemplateBundleFiles) => files['template.typ'] || '';

export const ensureDataBinding = (template: string) => {
  if (template.includes('json("data.json")') || template.includes("json('data.json')")) {
    return template;
  }
  if (template.includes('sys.inputs.data')) {
    return template;
  }
  return `#let data = json("data.json")\n${template}`;
};
